      ...
      for (let i = 0; i < 3; i++) {
        pending.push(
          await dispatch("agent-run-strict", {
            ...
            // is delivered. CI saw a 200ms body finish first and fail the
            // "no drained run COMPLETES" assertion below — that is the race
            // the runbook describes, not a drill failure. The body must
            // outlive cancel delivery for the drill to be testing cancellation
            // at all: a run that ends on its own proves nothing about drain.
            sleepMs: 3000,
