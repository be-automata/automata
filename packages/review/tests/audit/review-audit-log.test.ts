...
describe('createReviewAuditLog', () => {
  ...
  it('persists each of the six event types', async () => {
    ...
    for (const type of ALL_TYPES) assert.ok(seen.has(type), `missing type ${type}`);
  });
  ...
  it('AC-11: persists payload as JSON and round-trips fields', async () => {
    ...
    assert.deepEqual(rows[0].payload, { before: 'active', after: 'fixed', severity: 'warning' });
  });
  ...
  it('list() filters by type', async () => {
    log.emit({ type: 'review.started', repo: 'a/b', prNumber: 1, payload: {} });
    log.emit({ type: 'review.completed', repo: 'a/b', prNumber: 1, payload: {} });
    await log.flush();
  ...
  it('count() returns total row count', async () => {
    for (let i = 0; i < 5; i++) {
      log.emit({ type: 'review.started', repo: 'a/b', prNumber: i, payload: {} });
    }
    await log.flush();
  ...
  it('AC-11: single review pass produces correct event counts', async () => {
    ...
    log.emit({ type: 'review.started', repo: 'a/b', prNumber: 1, payload: {} });
    log.emit({ type: 'review.token_usage', repo: 'a/b', prNumber: 1, payload: {} });
    for (let i = 0; i < 3; i++) {
      log.emit({
        type: 'review.finding_lifecycle',
        repo: 'a/b',
