...
describe("review-audit-log — work.blocked event type (O1)", () => {
  ...
  it("list filters by repo + prNumber + type", async () => {
    ...
    log.emit({
      ...
      repo: "a/x",
      prNumber: 1,
      payload: { reason: "review_context_missing" },
    });
    log.emit({
      type: "work.blocked",
      repo: "a/x",
      prNumber: 2,
      payload: { reason: "agent_review_not_posted" },
    });
    log.emit({
    ...
    await log.flush();
    const rows = log.list(
      { repo: "a/x", prNumber: 1, type: "work.blocked" },
      10,
    );
    assert.equal(rows.length, 1);
  ...
  it("append-only triggers still raise on UPDATE / DELETE for work.blocked rows", async () => {
    ...
    log.emit({
      ...
      repo: "a/x",
      prNumber: 1,
      payload: { reason: "app_missing_issues_write" },
    });
    await log.flush();
