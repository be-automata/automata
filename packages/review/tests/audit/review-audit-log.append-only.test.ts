...
describe("review_audit_log append-only triggers", () => {
  ...
  it("AC-10: rejects UPDATE attempts and leaves the row unchanged", async () => {
    ...
    try {
      assert.throws(
        () =>
          db.exec(
            "UPDATE review_audit_log SET type = 'review.completed' WHERE id = 1",
          ),
        /append-only/i,
      );
      const row = db
        .prepare("SELECT type FROM review_audit_log WHERE id = 1")
        .get() as { type: string };
  ...
  it("AC-10: rejects DELETE attempts and leaves the row unchanged", async () => {
    ...
    try {
      ...
      const row = db
        .prepare("SELECT COUNT(*) AS n FROM review_audit_log")
        .get() as { n: number };
