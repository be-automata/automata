...
describe("severity-policy — ordering + defaults", () => {
  ...
  test("locked default policy = block:warning surface:warning (no withhold-only band; info is non-gating)", () => {
    assert.deepEqual(DEFAULT_APPROVE_SEVERITY_POLICY, {
...
describe("severity-policy — classifySeverities (default policy)", () => {
  const cases: Array<{
    ...
    {
      name: "[info] → clean (non-gating under the default policy)",
      severities: ["info"],
      tier: "clean",
    },
    ...
    {
      name: "mixed [info, warning] → block (max wins)",
      severities: ["info", "warning"],
      tier: "block",
    },
    {
      name: "mixed [info, info] → clean",
      severities: ["info", "info"],
      tier: "clean",
    },
    {
      name: "mixed [warning, error] → block",
  ...
  for (const c of cases) {
    test(c.name, () => {
      assert.equal(
        classifySeverities(c.severities, DEFAULT_APPROVE_SEVERITY_POLICY),
        c.tier,
      );
    });
  }
...
describe("severity-policy — classifySeverities (gate policy)", () => {
  ...
  for (const c of cases) {
    test(`${JSON.stringify(c.severities)} → ${c.tier}`, () => {
      assert.equal(
        classifySeverities(c.severities, GATE_SEVERITY_POLICY),
        c.tier,
      );
    });
  }
...
describe("severity-policy — applyApproveSeverityFloor", () => {
    ...
    const out = applyApproveSeverityFloor(
      {
        ...
        comments: [{ severity: "info" as const }],
      },
      P,
    );
    assert.equal(out.verdict, "approve");
  ...
  test("approve + [warning] → request_changes", () => {
    const out = applyApproveSeverityFloor(
      {
        verdict: "approve" as const,
        comments: [{ severity: "warning" as const }],
      },
      P,
    );
  ...
  test("approve + [error] → request_changes", () => {
    const out = applyApproveSeverityFloor(
      {
        verdict: "approve" as const,
        comments: [{ severity: "error" as const }],
      },
      P,
    );
    assert.equal(out.verdict, "request_changes");
  ...
  test("approve + untagged comment (defaults to info) → approve", () => {
    const out = applyApproveSeverityFloor(
      { verdict: "approve" as const, comments: [{}] },
      P,
    );
    assert.equal(out.verdict, "approve");
  });
  ...
  test("INVARIANT: an LLM comment is never upgraded, even with critical findings", () => {
    const intent = {
      ...
      comments: [{ severity: "critical" as const }],
    };
    const out = applyApproveSeverityFloor(intent, P);
  ...
  test("preserves extra intent fields (body, comments) when rewriting the verdict", () => {
    const intent = {
      ...
      body: "the summary",
      comments: [
        { path: "a.ts", line: 3, body: "fix", severity: "error" as const },
      ],
    };
    const out = applyApproveSeverityFloor(intent, P);
    assert.equal(out.verdict, "request_changes");
...
describe("severity-policy — toleranceToPolicy", () => {
  const cases: Array<{
    ...
    {
      tolerance: "error",
      policy: { blockSeverity: "error", surfaceSeverity: "warning" },
    },
    {
      tolerance: "warning",
      policy: { blockSeverity: "warning", surfaceSeverity: "warning" },
    },
    {
      tolerance: "info",
      policy: { blockSeverity: "info", surfaceSeverity: "info" },
    },
  ];
  for (const c of cases) {
  ...
  test("every policy is well-formed (surface rank <= block rank)", () => {
    for (const t of BLOCK_TOLERANCES) {
      ...
      assert.ok(
        SEVERITY_ORDER.indexOf(p.surfaceSeverity) <=
          SEVERITY_ORDER.indexOf(p.blockSeverity),
        `surface <= block for ${t}`,
      );
    }
  });
...
describe("severity-policy — applyApproveSeverityFloor (per-tolerance)", () => {
  ...
  test("only-downgrade invariant holds under all three tolerances", () => {
    for (const t of BLOCK_TOLERANCES) {
      ...
      assert.equal(
        applyApproveSeverityFloor(comment, policy).verdict,
        "comment",
        `comment untouched (${t})`,
      );
      const rc = {
