  ...
  it(`reason is always ≤ 500 chars when matched (${TRIALS} trials)`, () => {
    ...
    for (let i = 0; i < TRIALS; i++) {
      ...
      if (r.matched && r.reason !== null) {
        assert.ok(
          r.reason.length <= 500,
          `trial=${i}: reason length ${r.reason.length} > 500: ${body}`,
        );
      }
    }
  });
  ...
  it(`matched=true implies the (newline-stripped) body satisfies the grammar (${TRIALS} trials)`, () => {
    ...
    for (let i = 0; i < TRIALS; i++) {
      ...
      if (r.matched) {
        ...
        const ok =
          stripped === "break glass" || /^break glass:\s*\S/.test(stripped);
        assert.ok(
          ok,
          `trial=${i}: matched but stripped body fails grammar: ${JSON.stringify(stripped)}`,
        );
      }
    }
  });
