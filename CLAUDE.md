...
| HATCHET_IT=1 HOST_UID_IT=1 pnpm --filter @terragon/worker exec vitest run --no-file-parallelism src/agent-run/supersede.integration.test.ts src/agent-run/scheduling-health.integration.test.ts src/agent-run/box-lock.test.ts src/agent-run/uid-reaper.integration.test.ts | worker supersede-policy E2E, scheduling-health IT, the kernel box-lock drills, and the Linux-only uid-reaper kill drill (`HOST_UID_IT=1`, needs a CI-provisioned `automata-it-agent` account; skipped elsewhere) against dockerized hatchet-lite |
