...
description: >-
  ...
  settings.json permissions and hooks, commands/skills, custom agents, and
  the autotest-to-green-PR lifecycle — then returns a /100 score, a maturity
  band, and a prioritized action plan. Read-only: never modifies the audited
  repo. Use when the user asks to audit the AI harness, score their Claude
  setup, assess AI adoption maturity, or check how paved the quality path
  is. Triggers on: 'harness audit', 'ai harness', 'claude harness score',
