...
- **BUG-EXEC-02: STILL OPEN**, deploy-ineffective (wrong-base-field). S15 pre-fix baseline (false-approve) + this post-deploy-still-broken are both correct records. Next re-run bar (team-lead) adds: assert the WORKER LOG shows "(base main fetched)" (`workflow.ts:76`) — deterministic proof the base reached provision, upstream of the agent's diff. HOLDING the re-run for team-lead's next deploy-live ping (after tenancy-coder's re-source).
