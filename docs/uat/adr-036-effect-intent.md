...
So (a) is RULED OUT: S3's worktree did NOT have the base; the APPROVE was diff-free. Cross-run object-cache also ruled out (provision rm+fresh-clones per threadId). **The base is ALWAYS unavailable on synchronize (deterministic); the COMMENT-vs-APPROVE swing is AGENT-RESPONSE nondeterminism on identical broken input** — one run honestly declines (COMMENT), another reviews HEAD content and reaches a verdict that never saw the delta. Two consequences that nudge this above a plain "occasionally unavailable": (i) a content-only APPROVE never saw the incremental delta, so it could MISS a regression confined to lines/files the agent didn't happen to Read (it is NOT a false-bless of a defect it saw, but it is a weaker approve on incomplete input); (ii) every re-review eats a doomed 2-min fetch hang. Still capped at medium: no false verdict on seen code, and it fails toward COMMENT/incomplete-approve, not a wrong CHANGES_REQUESTED or a bless of a visible defect. FIX (team-lead ruling, aligned to operator's "raw data + tools, not hardcoded fetchers"): the TOKENED worker deepens-to-merge-base at clone time (before the strip) so the agent's OWN `git diff base...HEAD` works offline [boot-coder]. NOT a www diff-prefetch (team-lead retracted that approach). Queued to the post-green PR as its own slice. NOT a harness artifact (fixture commits carry full origin/main parentage — verified).
