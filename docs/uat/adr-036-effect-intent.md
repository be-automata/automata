...
Severity: **medium, pending boot-coder's S3 agent-tool-trace to lock (b) airtight.** The clone-command evidence (`--depth 1 --branch <head>`, single hardcoded clone for every run) already rules out (a) at the code level — S3's worktree was necessarily base-less too — so (b) is near-certain: S3 approved diff-free from clean content. The one remaining confirmation is S3's agent trace showing a Read-and-approve with no successful `git diff`. If (b) holds (expected), it is deterministic: EVERY delta-needing re-review degrades until fixed (worse than "intermittent"); still safe-degrade (never a false verdict) → capped at medium. FIX (team-lead ruling, aligned to operator's "raw data + tools, not hardcoded fetchers"): the TOKENED worker deepens-to-merge-base at clone time (before the strip) so the agent's OWN `git diff base...HEAD` works offline [boot-coder]. NOT a www diff-prefetch (team-lead retracted that approach). Queued to the post-green PR as its own slice. NOT a harness artifact (fixture commits carry full origin/main parentage — verified).
