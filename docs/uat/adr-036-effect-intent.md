...
- **Verdict rubric:** PASS iff **all N issues answered, one run per issue**. Currently **FAIL (real concurrency bug)**.
  - **Investigation history (three attempts — read as a caution against premature root-causing):**
    ...
    3. **Attempt 2 (issues #15-18, NEUTRAL fixtures — genuine questions):** still FAIL, but now a **REAL bug the framing had masked**. Independently verified GitHub-side + boot-coder OLAP correlation: 4 distinct neutral mentions all got the eyes-ack, but **only 3 agent-runs dispatched**, and **two collapsed onto issue #15** (2 replies @15:45:08 + @15:46:18) while **#16/#17/#18 got zero run and zero reply**. Net **1/4 issues answered**. NOT fixture-framing (questions genuine, #15 fully answered), NOT token-laundering (runs COMPLETED, replies posted).
  - **2026-07-18 VERDICT: FAIL — burst concurrency mis-routing / under-dispatch.** Under a near-simultaneous burst, mentions collapse (multiple runs onto one issue) and siblings get no run at all.
  - **Root cause (hypothesis — www confirmation pending, boot-coder + tenancy-coder):** the shared `legacy-thread-chat-id` sentinel collapsing thread/message routing under burst (thread creation or `getThreadChat`'s userId+threadChatId fencing — upstream of the threadId-keyed next-message pull), possibly compounded by `MAX_CONCURRENT_TASKS_PER_USER=3` truncating the 4th mention. Same sentinel class as F2, but the collapse is at thread creation/routing, not token binding.
  - **Flip to PASS when:** the mis-routing/under-dispatch fix deploys and a neutral-fixture burst re-run yields all N answered, one run per issue. Then the block closes 10/10.
  - **LESSON baked into this case:** a test-framed fixture (attempt 1) masked a real concurrency bug (attempt 2) AND lured two wrong root causes. Always: neutral fixtures + read the agent's own transcript before theorizing.
