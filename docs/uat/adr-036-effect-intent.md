...
## FLAG-FLIP ACCEPTANCE — RUN 1+2 root cause: TWO issues, agent never launched (2026-07-21)
Both the fresh-run gate attempts degraded (PR #23 swaccept, PR #24 swaccept2 — both "verdict NOT applied"). The single-writer channel/executor/fallback/safety PASSED both times; the AGENT emitted no intent. Two separate root causes, both needed:
1. **[PRIMARY] Shell-quote bug (boot-coder 41e8f3a):** the daemon spawns claude via `bash -c`; the review-policy `--disallowedTools Bash(gh:*) Bash(git push:*)` matchers were UNQUOTED → `bash: -c: syntax error near unexpected token '('` (thread b56dc4a7 error_info) → **the agent NEVER LAUNCHED** → zero output → executor correctly degraded. Fix committed to origin/main; **NEEDS A WORKER REDEPLOY** (daemon dist rebuild) — not yet live when swaccept2 ran.
2. **[SECONDARY] Stale automation instruction (team-lead, live-fixed):** the deployed automation row c95b9307 still carried old prose ("prod skill: github-ops"), NO fenced-JSON contract — tenancy-coder's seed (f41fbbc) is create-if-not-exists ("Skipped exists") so the live Neon row never updated. **The "automations are runtime DB state, not code" class** (same as the mention-automation seed gap). team-lead patched the live row directly (1113 chars, contract present).

**Evidence #1 is primary:** swaccept2 ran AFTER the automation-row fix but BEFORE the shell redeploy → STILL degraded. So the agent-never-launches (shell) bug dominates; the contract fix can't help until the agent runs.
**Validator note (own it):** I re-fired swaccept2 before confirming 41e8f3a was redeployed — violated my own "confirm the fix is deployed before probing" rule; degraded a 2nd time avoidably. HOLDING the 3rd re-fire until the worker kickstart with 41e8f3a is CONFIRMED live. Both fixes must be live: 41e8f3a redeployed + the automation row (done).
**Silver lining:** RUN 1+2 proved the safety net live TWICE — executor posts exactly one, degraded marker visible, WorkFailed loud, reconciler-behind, no silent drop, no dup — exactly the code-verified behavior, under a real agent-never-launched failure. The gate's SAFETY half is now live-proven; the HAPPY-PATH (real verdict) awaits the redeploy.

## FLAG-FLIP ACCEPTANCE — RUN 3 (swaccept3/PR #25): STILL degraded post-kickstart; correction (2026-07-21)
**Root-cause correction:** the earlier "two root causes" is WRONG — it's ONE cause. team-lead confirmed the automation instruction propagated (PR#24 agent message = 2111 bytes with the full fenced-JSON contract), so the instruction was never the problem. The sole cause is the daemon shell-spawn: `bash -c "…--disallowedTools Bash(gh:*)…"` with UNQUOTED parens → `bash: -c: syntax error near unexpected token '('` → the agent NEVER LAUNCHES → no output → executor correctly degrades.
**RUN 3 finding:** boot-coder committed the quote fix (41e8f3a) + kickstarted the worker (20:28:42) + verified the quoted matcher in dist. I fired the clean re-fire swaccept3 (PR #25) on that confirmation — and it DEGRADED AGAIN, identical signature: "no JSON intent block found in agent output"; PR open 00:32:38 → run 8ce56379 COMPLETED 00:32:45 (7s) → degraded 00:33:06. So **the fix is in the committed dist but NOT live-effective** — the running daemon still fast-degrades. Handed back to boot-coder to check: did the kickstart actually replace the serving process? is the fix complete (also quote `Bash(git push:*)`)? does thread 8ce56379 error_info still show the SAME bash syntax error (= fix not live) or a new one?
**Validator lesson (reinforced):** "the fix is in the dist" ≠ "the running daemon uses it" ≠ "behavior fixed." Only a live probe confirms — swaccept3 was that probe, and it correctly caught that the kickstart didn't take. HOLDING all re-fires until a direct daemon-level agent-launch is confirmed (not dist-only). PRs #23/#24/#25 cleaned up.
**Standing positive:** the single-writer executor + degraded-fallback + WorkFailed + reconciler-behind + no-dup have now fired correctly across THREE agent-never-launched runs — SAFETY half live-proven 3×; the happy-path verdict still blocked on the daemon spawn.
**Superseded by RUN 4:** the "it's ONE cause (shell only)" conclusion here was itself incomplete — RUN 4 found the SECOND live blocker (the launchd worker PATH gap, claude-not-found) that RUN 3 could not yet see. Read RUN 3 as the then-current understanding; the final two-blocker resolution is in RUN 4 below.
...
**BUG-EXEC-02 (OPEN, tracked): re-review verdict-fidelity blocked when the verdict needs the base delta.** S2 (partial-fix synchronize) posted a REAL, parse-clean COMMENTED (not degraded) because the agent's review worktree had no base to diff against ("single orphan commit, no cached remote refs", `git fetch` unavailable in the token-less sandbox) → the agent correctly declined to guess and COMMENTed. boot-coder's process-side root-cause: provision.ts clones with `git clone --depth 1 --branch <branch>` (shallow, branch head only, no base) → the token-less agent can't compute `base...HEAD`.

CHARACTERIZATION — SETTLED by boot-coder's provision.ts + process trace (my two earlier framings were both over-extrapolations, recorded as the lesson): (1) "blocks the whole re-review path" — WRONG, S3 passed; (2) "intermittent (timing/cache)" — ALSO WRONG. Resolved answers:
- **Deterministic, NOT intermittent.** provision.ts:39-53 clones `git clone --depth 1 --branch <head>` — always shallow, single orphan commit, base ref never fetched. Candidate (a) "clone depth varies" is RULED OUT.
- **Why S3 passed anyway = candidate (b).** A clean-file APPROVE is reachable from file-content-at-HEAD without a diff (S3's fully-fixed file has no console.log and `>= 18` → both prior asks verifiable as addressed by Read alone). S2's MIXED partial-fix state (one ask addressed, off-by-one remaining) is the case whose verdict genuinely needs the base delta → no base → honest COMMENTED. So the gap bites every re-review whose verdict needs the DELTA, not a clean-approve.
- **The "timeout" is NOT network egress** (my earlier read was wrong): base-fetch is expected to work offline; the token-less agent's private-repo `git fetch` hangs on the git credential prompt (no tty, no credential) until timeout — reads as "timeout", cause is the missing credential.
- **Pre-existing AND unmasked by the token-strip (not a token-strip bug to "fix" by restoring the token).** The shallow clone was always base-less; before single-writer the agent had GH_TOKEN + git extraheader and could `git fetch` the base to self-heal on re-review; the strip removed that fallback and exposed the latent defect. Restoring the token would defeat single-writer — the real fix is worktree-prep.
Severity: **medium — LOCKED to (b) deterministic by boot-coder's tool-call trace (2026-07-21).** The airtight datum is in: both re-review threads' actual tool RESULTS show the base was unavailable identically —
- S2 (thread f8d59e07 @ de85626e → COMMENT): `git diff origin/main...HEAD` = exit 128 "unknown revision"; `git fetch origin main` = exit 143 "timed out after 2m".
- S3 (thread adcc2e98 @ 50129b47 → APPROVE): `git fetch origin main` = exit 143, the EXACT same 2m timeout — then the agent Read HEAD content (lib.ts, `git log --follow -p` on the fixture) and approved from THAT, never a diff.
So (a) is RULED OUT: S3's worktree did NOT have the base; the APPROVE was diff-free. Cross-run object-cache also ruled out (provision rm+fresh-clones per threadId). **The base is ALWAYS unavailable on synchronize (deterministic); the COMMENT-vs-APPROVE swing is AGENT-RESPONSE nondeterminism on identical broken input** — one run honestly declines (COMMENT), another reviews HEAD content and reaches a verdict that never saw the delta. **TWO failure modes on this deterministic-broken input, and the second is NOT fail-safe (record corrected — the earlier "never a false verdict / safe-degrade" framing was WRONG, per team-lead):**
1. **Decline → marked COMMENT** (S2-style): visible, fail-safe. ✓
2. **Diff-free APPROVE** (S3-style): the agent approves from whatever HEAD files it happened to Read, WITHOUT the delta. Because it has no diff, it CANNOT know which files the synchronize changed — so on a partial fix whose still-unaddressed defect sits in a file the agent doesn't Read, it can FALSE-APPROVE a defective PR. S3 approved correctly ONLY because the fixture was fully fixed; the general case includes a real false-bless path. This is NOT safe-degrade.

Severity stays **MEDIUM** but bounded, NOT "purely fail-safe": the agent does Read files + uses `git log --follow -p`, so it isn't blind — a false-approve requires the missed defect to be outside everything it Read. Plus every re-review currently burns a doomed 2-min fetch hang (perf/cost). Both consequences make the queued fix MORE warranted. (Related: fork-PR handling → BUG-EXEC-03, boot-coder filing separately.) FIX (team-lead ruling, aligned to operator's "raw data + tools, not hardcoded fetchers"): the TOKENED worker deepens-to-merge-base at clone time (before the strip) so the agent's OWN `git diff base...HEAD` works offline [boot-coder]. NOT a www diff-prefetch (team-lead retracted that approach). Queued to the post-green PR as its own slice. NOT a harness artifact (fixture commits carry full origin/main parentage — verified).

**BUG-EXEC-02 RE-RUN TEST DESIGN (ready for when the worker base-fetch fix deploys).** Two cases:
- **S2-rerun (verdict-fidelity):** re-run `pnpm uat S1` (drives S1-S3). Assert S2 now returns a DETERMINISTIC still-CR at the partial-fix HEAD (catches the remaining off-by-one via a real `base...HEAD` diff), NOT COMMENTED and NOT a diff-free APPROVE. Pre-fix baseline already on record (RUN 4: S2=COMMENTED).
- **S16 false-approve case (new, multi-file — proves mode-2 empirically + that the fix kills it):**
  - Fixture: PR adds TWO files — `fixtures/mf-a.ts` (defect1: off-by-one `age > 18`) and `fixtures/mf-b.ts` (defect2: `console.log("API key:", k)` secret-log). Opened → CR (both defects; at open, `git log -p` of the single PR commit shows both files, so the agent sees both).
  - Synchronize = PARTIAL fix touching ONLY mf-a (fix the off-by-one); mf-b's secret-log LEFT defective, and the fix commit does NOT touch mf-b.
  - **Pre-fix expected (baseline, if team-lead wants it): FALSE-APPROVE or COMMENT** — base-less, the agent's `git log -p`/HEAD-content review sees only the latest commit (mf-a, now clean) and cannot know mf-b is part of the unmerged PR delta → approves without re-reviewing mf-b. Assert: verdict != CHANGES_REQUESTED (the false-bless observed), captured as the BUG-EXEC-02 mode-2 evidence.
  - **Post-fix expected: CHANGES_REQUESTED** — with the base fetched, `base...HEAD` shows BOTH files as PR changes; the agent re-reviews mf-b, catches the still-present secret-log → blocks. Assert: exactly one CR at HEAD naming mf-b's secret-log; confirms the fix closes the false-approve path.
  - Note: mode-2 is agent-response-nondeterministic, so the pre-fix run may COMMENT instead of false-APPROVE on a given attempt — either outcome is "not the correct CR" and validates the gap; a false-APPROVE specifically is the stronger evidence. Fixture common to both the lean (post-fix-only) and baseline (pre+post) options — only the run timing differs. Awaiting team-lead's lean-vs-baseline call + boot-coder's fix deploy.

**S15 PRE-FIX BASELINE — FALSE-APPROVE OBSERVED (2026-07-21, PR #28, on the pre-fix worker pid 55661).** The mode-2 false-bless is no longer a logical argument — it is an observed run. Lifecycle on PR #28:
- Opened @875e256b → CHANGES_REQUESTED correctly naming BOTH defects (mf-b secret-log + mf-a off-by-one).
- Synchronize = partial-fix of mf-a ONLY → **APPROVED @16766ae7**: "The PR fixes the off-by-one in isAdult (mf-a) ... isolated fixture file" — the agent approved on the mf-a fix alone, **never mentioned mf-b**, whose secret-log is STILL PRESENT, and **dismissed the prior CHANGES_REQUESTED** that had correctly flagged it.
So a defective PR (unaddressed secret-logging) was AUTO-APPROVED and its blocking review dismissed, precisely because the base-less agent could not see mf-b as part of `base...HEAD` and reviewed only the latest commit's file. S15 = FAIL (the guard firing as designed). This is the empirical confirmation that BUG-EXEC-02 mode-2 harms a real merge decision — the datum team-lead wanted observed, not argued. Post-fix expectation: S15 flips to PASS (CR catching mf-b via a real base diff). The permanent S15 case now stands guard against any reintroduction.

**RUN 5 — POST-DEPLOY E2E: BUG-EXEC-02 fix DEPLOYED but INEFFECTIVE; S15 false-pass caught + guard hardened (2026-07-21).** After team-lead deployed the "complete" fix (www Version 85c5fa59 baseBranch dispatch + worker c38776a + skill primary-path + automation upsert) and kickstarted the post-fix worker (21:52:29), I ran `pnpm uat S15 S1` on it. Result: the fix is INEFFECTIVE — do NOT record BUG-EXEC-02 closed.
- **S1/S2/S3 = FAIL** (PR #29): every review COMMENTED "origin/main is not present in this clone / could not compute git diff origin/main...HEAD". The base is STILL not fetched. Note the skill-hardening (HEAD~1 demoted to a warning) made even the OPENED path stricter, so S1-opened now degrades to COMMENTED too (pre-deploy it passed via content review) — the base-fetch must work for ANY diff-needing verdict.
- **ROOT CAUSE (team-lead → tenancy-coder): wrong base field.** dispatch.ts sourced `baseBranch` from `thread.repoBaseBranchName`, but on a PR-review thread that field holds the HEAD branch, not the base (PR#29 `repo_base_branch_name = "uat/adr036-s1s3-e2epostfix"`, NOT "main"). So the `!==branch` guard fails → baseBranch undefined → worker skips the base-fetch → origin/main absent. The real base is at `handlers.ts:678` (`pull_request.base.ref`), not yet plumbed. tenancy-coder re-sourcing.
- **S15 FALSE-PASS caught (the important validator lesson).** My S15 first reported PASS — the synchronize returned CHANGES_REQUESTED naming mf-b. But the CR body ADMITS: "I could not compute git diff origin/main...HEAD: origin/main is not present locally ... However, this PR's file tree at HEAD is fully readable, and it clearly contains the two-file defect pair." So mf-b was caught by CONTENT-READ of the file tree at HEAD — luck of which files the agent read — NOT via base...HEAD. The base-diff mechanism was BROKEN, yet S15 greened on outcome. **This is exactly the false-pass team-lead's mechanism bar exists to catch** ("a green could come from the agent guessing right, the fallback, or luck"). Hardened S15 (`scripts/uat/cases.ts`): a TRUE pass now requires the blocking review to name mf-b AND NOT admit a diff failure (`could not compute` / `origin/main is not present`); an mf-b catch that admits the diff failed is recorded FAIL "MECHANISM NOT PROVEN". With the hardened guard, RUN 5's S15 = FAIL (correctly).
- **BUG-EXEC-02: STILL OPEN**, deploy-ineffective (wrong-base-field). S15 pre-fix baseline (false-approve) + this post-deploy-still-broken are both correct records.
- **SEVERITY UPGRADE (team-lead, 2026-07-21): medium → CRITICAL-PATH / high.** The skill-hardening (HEAD~1 demoted to a warning) removed the agent's content-review fallback, so on the deployed plane a broken base-fetch degrades EVERY verdict — opened AND synchronize — to COMMENTED. BUG-EXEC-02 is therefore no longer a re-review-only robustness gap: it currently BLOCKS ALL REVIEW VERDICTS on the live plane (every review COMMENTED). The base-fetch is now load-bearing for reviews working at all — the fix is on the critical path, not a hardening nicety. (The skill hardening itself is correct — once the base-fetch works, every review gets a real diff, strictly better; the sole problem is the broken base-fetch.)
- **EXPANDED RE-VERIFICATION BAR (next post-fix run, after tenancy-coder re-sources baseBranch from `pull_request.base.ref`):** confirm BOTH (a) S1-OPENED gets a real base diff → correct CR (NOT COMMENTED), and (b) S15/S2 re-review blocks correctly (hardened mechanism guard: names mf-b AND no diff-failure admission) — PLUS the worker-log "(base main fetched)" (`workflow.ts:76`, logs only when baseBranch is set) as the upstream deterministic proof on BOTH, and boot-coder's agent-trace `git diff origin/main...HEAD` exit-0 downstream. HOLDING the re-run for team-lead's deploy-live ping.

**RUN 6 — BUG-EXEC-02 CLOSED: re-source fix WORKS, mechanism-proven both sides + single/multi-file (2026-07-21).** team-lead re-sourced baseBranch from the PR's real base ref (`pull_request.base.ref` via handlers.ts:678, not `repoBaseBranchName`), deployed www Version e98e4bb7 (HEAD c82b9e1), kickstarted the worker. I ran `pnpm uat S15 S1` on it. **ALL PASS (S1/S2/S3/S15 = 4/4)** — and held to the mechanism bar, NOT outcome alone (this is the same deploy shape that was INEFFECTIVE in RUN 5, caught only by the mechanism check):
- **UPSTREAM proof (I confirmed in ~/.automata/worker.log):** every e2ev2 clone-complete carries "(base main fetched)" — for BOTH PR#31 (s1s3) and PR#32 (s15). The exact suffix whose ABSENCE proved RUN 5 inert is now present. baseBranch=main reached provision; the worker fetched origin/main + deepened.
- **DOWNSTREAM proof (boot-coder, Neon tool-results):** `git diff origin/main...HEAD` is_error=FALSE (exit-0) with real delta on the synchronize runs — S2 (e6709e70 @06aacde) and the S15 partial-fix (75e360ef @41b0ef46). Not exit-128, not a fetch timeout, not content-read.
- **VERDICT FIDELITY (diff-based, not content-guess):** S2 request_changes recognized the partial fix removed only the console.log and the off-by-one was untouched → blocked. S15 synchronize request_changes: "mf-a.ts fixes the isAdult off-by-one correctly (age >= 18). However, mf-b.ts introduces logKey(), which logs a secret/API key to the console ... a PR that logs credentials must never be approved" → blocked on the UNTOUCHED file's defect via the real diff.
- **My HARDENED S15 guard PASSED honestly:** evidence `mechanismProven:true, baseDiffAdmittedFailed:false, catchesMfB:true`; the synchronize CR (@41b0ef46) names mf-b AND does not admit a diff failure. The hole I closed in RUN 5 (content-read false-pass) now confirms the REAL fix rather than masking a broken one.
- **CLEAN BEFORE/AFTER on the mode-2 false-bless:** pre-fix baseline PR#28 (partial-fix synchronize → APPROVED, never mentioned mf-b, dismissed the prior CR) → post-fix PR#32 (partial-fix synchronize → request_changes naming mf-b via a real base diff). Regression KILLED and now permanently guarded by S15.

**BUG-EXEC-02: CLOSED** (proven, not lucky-green — the mechanism bar + hardened guard are what make it a real close). All review verdicts (opened + synchronize, single- and multi-file) now run on a real `git diff origin/main...HEAD`. Related fork-PR handling BUG-EXEC-03 remains boot-coder's separate item.
...
- S10/S11/S13/S14 (inline-thread surface) remain gated on a SEPARATE flag, REVIEW_POST_INLINE_COMMENTS — PR#27 posted 0 inline comments (findings folded into the review body), so that surface is not live; these stay SKIPPED, NOT force-passed by the single-writer flip. Diff-dependency breakdown (per team-lead's ask):
  - **S14 (one-review-object): CHANNEL-ONLY** — substantively ALREADY covered by RUN 4's proven no-dup invariant (exactly one review object per commit). The only unproven remainder is the inline-comment packaging (one review carrying N inline comments vs N reviews), which needs REVIEW_POST_INLINE_COMMENTS. Not diff-dependent.
  - **S10 (inline threads), S11 (reply-then-resolve): need the inline surface AND the diff** — inline comments are anchored to diff lines, so on any re-review they are ALSO subject to BUG-EXEC-02. Blocked on both REVIEW_POST_INLINE_COMMENTS (off) and the base-delta fix.
  - **S13 (stale-thread re-raise): needs the diff** — re-raising an unresolved thread on a new commit is verdict-aware idempotency across commits (same shape as S2/S3), so it hits BUG-EXEC-02 directly; also needs the inline surface for the thread object. Filed under BUG-EXEC-02, not a new finding.
