...
Severity: medium — safe-degrade (never a false verdict), but deterministic means EVERY delta-needing re-review degrades until fixed. FIX (team-lead ruling, aligned to operator's "raw data + tools, not hardcoded fetchers"): the TOKENED worker deepens-to-merge-base at clone time (before the strip) so the agent's OWN `git diff base...HEAD` works offline [boot-coder]. NOT a www diff-prefetch (team-lead retracted that approach). Queued to the post-green PR as its own slice. NOT a harness artifact (fixture commits carry full origin/main parentage — verified).
