...
CHARACTERIZATION IS NOT YET SETTLED — two of my successive framings were over-extrapolations, flagged here to avoid a third: (1) "blocks the whole re-review path" — WRONG, S3 passed; (2) "intermittent (timing/cache)" — UNPROVEN, and it contradicts boot-coder's deterministic `--depth 1` read. The unresolved tension: if the clone is ALWAYS shallow, why did S3 (full-fix) reach a substantive APPROVE? Two candidates, distinguishable only by boot-coder's S3 process trace, NOT by GitHub-side evidence: (a) S3's worktree DID have the base (clone depth varies → genuinely intermittent), or (b) deterministic-shallow but a clean-file APPROVE is reachable from file-content-at-HEAD without a diff (S3's fully-fixed file is verdict-able whole; S2's mixed addressed/unaddressed state needs the delta). Deferred to boot-coder as trace-holder. NOT a harness artifact (fixture commits carry full origin/main parentage — verified) and NOT the token-strip (missing base is clone-depth, not auth). Severity: medium — safe-degrade (never a false verdict), but if (b)/deterministic then EVERY delta-needing re-review degrades, which would raise it. Fix (either eliminates it): tokened worker fetches the base ref at clone time so `base...HEAD` works offline [boot-coder], and/or www prefetches the diff into the review instruction [tenancy-coder].
