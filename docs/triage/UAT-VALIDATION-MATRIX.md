...
## S12 ROOT-CAUSE CORRECTION #2 — mis-routing DISPROVEN; real cause = QUEUE STARVATION (team-lead www trace) (2026-07-18)

My attempt-2 "concurrency mis-routing / collapse onto #15 / sentinel" verdict (67693f6) is RETRACTED — team-lead's www routing trace (scratchpad/burst2.txt) disproves it. Routing + thread creation were CORRECT; the sentinel is not involved. Actual:
- **Fixture confound (harness bug — FIXED here):** each freshly-created issue ALSO fires the issue-research automation (`8304af2f`, on open), so 4 issues = **8 intents** (4 research + 4 mention). #15's two replies = a research response + a mention answer (both legitimate); the run that "posted nothing" was an issue-research run. No dispatch collapse, no duplication. Harness S12 now isolates the mention path (create issues → 90s settle → burst mentions → count replies after the mention timestamp).
- **REAL defect (S12 FAIL stands):** the per-user capacity gate (`MAX_CONCURRENT_TASKS_PER_USER=3`) engaged CORRECTLY — 3 intents dispatched (threads `14aeb04c/4620eed5/7eed4c86`), intents 4-8 went `[system.concurrency-limit] queued → queued-tasks-concurrency` (threads `361538f8/b6148530/bc248f0e/61404487/a393188b`) — **and nothing ever dequeues them.** Queue was already **10-deep pre-burst** (silently accumulating across all testing), **14 now**. tenancy-coder investigating the missing drain (quarantined Redis/cron; `evalsha` TypeError). No user "at capacity" feedback either.
- **VERDICT: S12 FAIL — over-capacity work queues and STARVES forever (no drain, no feedback).** This is the well-evidenced form of exactly what OLD orch-agents S12 tested.
...
## ✅ S12 PASS + BUG-EXEC-01 CLOSED + BLOCK 10/10 — FINAL (team-lead sign-off) (2026-07-18)

**S12 = PASS.** Invariant proven live on the fixed plane (www 8622f4e4): a 4-mention over-capacity burst — every one of the 4 mention threads **promoted out of `queued-tasks-concurrency` and got answered**, no silent loss. GitHub-side conclusive evidence: all 4 issues #19-22 reached their full **2 bot replies each** (research thread + mention thread; 2 automations/issue → 2 threads → both completed) by 17:17:50Z. boot-coder's OLAP corroborates per-mention-thread promotion (research all COMPLETE; mention threads promoted booting→answered). Pre-fix these 4 mention threads starved forever.
- **Harness note:** wall-clock ~13 min (17:04 burst → 17:17 all answered), throughput-bound on the concurrency=1 pilot worker + slow issue-research runs hogging the single slot — a SUBSTRATE property, not the invariant. Also a measurement confound: creating issues fires BOTH the issue-research (`8304af2f`, on-open) and mention (`d24bfe58`) automations; the hardened harness (57b6a85) waits for research to settle then asserts post-mention, and the intended shape (approved) is issue_comment on pre-existing issues + a pilot-sized timeout.
- **KNOWN-LIMITATION (not a defect):** pilot Hatchet worker concurrency=1 (fixed daemon socket) caps throughput. Per **ADR-002 §6**, worker concurrency is the CUSTOMER'S capacity knob — a real deployment sizes it. This is a capacity-spec fact.

**BUG-EXEC-01 = CLOSED.** Fix evidence chain: Part 1 native cron triggers wired via Workers `scheduled()` (4c62587 + c0a8444) — all 4 crons fire their in-process runners; Part 2 in-process finish-promotion + stale-retirement, last internalPOST self-fetch killed (09e4a76). Result verified live: queue drained 14→0 (backlog retired, not executed), over-cap work promotes on slot-free, all 4 mention threads answered. Both required halves live; no re-stranding.
