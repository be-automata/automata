 ...
 * removes direct git from non-review lanes, and `api.anthropic.com` for
 * box-key runs. Merged into the shape control-plane-side so planes receive a
 * FINAL allowlist and never compute system entries themselves.
 */
function systemEgressHosts(): string[] {
  const callbackHost = new URL(nonLocalhostPublicAppUrl()).host;
  return [callbackHost, "github.com", "api.anthropic.com"];
}
