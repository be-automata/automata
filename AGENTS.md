...
- `.claude/agents/` and `.codex/agents/` — repo-local subagent definitions. Claude Code reads
  `.claude/agents/**/*.md` (`code-reviewer`, `senior-code-reviewer`, `docs-engineer`,
  `yeet-responder`, plus `testing/production-validator` and `workflow/pr-manager`); Codex reads
  `.codex/agents/*.toml`, which mirror the first four with the same `name` and `description`
  and the Markdown body carried in `developer_instructions`. These are hand-maintained, not
  generated by `harness-init`, so a change to an agent must be applied to both files.
